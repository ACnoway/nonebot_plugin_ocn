...
async def get_today():
    ...
    if len(results) == 0:
        ...
        for conts in results:
            ans += "<li>\n<ul>"
            ans += f"\n<li>比赛平台：{ojs[conts[0]][0]}</li>"
            ans += f"\n<li>比赛名称：{conts[1]}</li>"
            ans += f"\n<li>比赛时间：{conts[2]}</li>"
            ans += f"\n<li>比赛链接：{ojs[conts[0]][1]}{conts[3]}</li>"
            ans += "</ul>\n</li>\n"
            counts -= 1
            if counts == 0:
                break
        ans+="</ol>"
        if counts > 0:
            return kaitou + f'近期比赛不足 {str(counts)} 场，找到近期 {str(len(results))} 场比赛如下：\n' + ans + f'\n\n防风控编码 {str(time.time())}\n'
        else:
            return kaitou + f'找到近期 {str(len(results))} 场比赛如下：\n' + ans + f'\n\n防风控编码 {str(time.time())}\n'
